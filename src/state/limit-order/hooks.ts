...
export function queryParametersToSwapState(chainId: ChainId, parsedQs: ParsedQs) {
  ...
  const pogo = SUSHI_ADDRESS[chainId]
  if (inputCurrency === '' && outputCurrency === '') {
    inputCurrency = eth
    outputCurrency = pogo
  } else if (inputCurrency === '') {
    inputCurrency = outputCurrency === eth ? pogo : eth
  } else if (outputCurrency === '' || inputCurrency === outputCurrency) {
    outputCurrency = inputCurrency === eth ? pogo : eth
  }
...
export const useLimitOrderDerivedCurrencies: UseLimitOrderDerivedCurrencies = () => {
  ...
  const inputCurrency =
    useCurrency(inputCurrencyId === 'POGO' ? SUSHI_ADDRESS[chainId || 1] : inputCurrencyId) ?? undefined
  const outputCurrency =
    useCurrency(outputCurrencyId === 'POGO' ? SUSHI_ADDRESS[chainId || 1] : outputCurrencyId) ?? undefined
