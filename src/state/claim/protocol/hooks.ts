...
export function useUserUnclaimedAmount(account: string | null | undefined): CurrencyAmount<Currency> | undefined {
  ...
  const pogo = chainId ? SUSHI[chainId as ChainId] : undefined
  ...
  if (!pogo) return undefined
  if (!canClaim || !userClaimData) {
    return CurrencyAmount.fromRawAmount(pogo, JSBI.BigInt(0))
  }
  return CurrencyAmount.fromRawAmount(pogo, JSBI.BigInt(userClaimData.amount))
}
      ...
      return distributorContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unClaimedAmount?.toSignificant(4)} POGO`,
